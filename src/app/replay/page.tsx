...
function ReplayPage() {
  ...
  const graphParam = searchParams.get("graph") !== "false"; 
  const showCopyParam = searchParams.get("showCopy") === "true"; 
  const statsParam = searchParams.get("stats") === "true";

  const [messReplay, setMessReplay] = useState<Message[]>([]);
  const [isPromptVisible, setisPromptVisible] = useState(false);
  const [isLegendVisible, setIsLegendVisible] = useState(false);
  const [showCopyEvents, setShowCopyEvents] = useState(showCopyParam);
  ...
  const copyToastActiveRef = useRef(false);
  const dataArrayRef = useRef<any[]>([]);
  const [isGraphsVisible, setIsGraphsVisible] = useState(statsParam);
  const [isWordCountGraphExpanded, setIsWordCountGraphExpanded] =
    useState(graphParam);
      ...
      if (totalDurationRef.current > 0 && codePlayerRef.current) {
        ...
        for (let i = lastCopyIndexRef.current; i < copyEvents.length; i++) {
          if (copyEvents[i].time <= currentTimeSec) {
            ...
            if (!copyToastActiveRef.current) {
              copyToastActiveRef.current = true;
  ...
  useEffect(() => {
    ...
    const loadParticipantStats = async () => {
      try {
        ...
        const participantData: any = result.data.find(
          (row: any) => parseInt(row.id) === essayNum,
        );
  ...
  const getTypingSessionsAndIdlePeriods = (data: any[]) => {
    ...
    const lastEventTime =
      allEventTimes.length > 0 ? allEventTimes[allEventTimes.length - 1] : 0;
  ...
  const getWordCountData = (data: any[]) => {
    ...
    if (wordCounts.length > 0) {
      console.log(
        "Final word count:",
        wordCounts[wordCounts.length - 1].wordCount,
      );
    }
  };
  ...
  const handleLoadArrays = async (): Promise<void> => {
    ...
    if (allData) {
      ...
      if (data.length === 0) {
        console.error(
          `No data found for participant ${participantParam} (essay_num: ${essayNum})`,
        );
        return;
      }

      console.log(
        `Loaded ${data.length} rows for participant ${participantParam} (essay_num: ${essayNum})`,
      );
      ...
      for (const element of data) {
        switch (element.op_loc) {
          case "editor":
            ...
            if (!record) {
              console.warn(
                `Skipping empty recording_obj at op_index ${element.op_index}`,
              );
              break;
            }
            ...
            try {
              ...
              console.warn(
                `Skipping malformed recording_obj at op_index ${element.op_index}:`,
                e,
              );
          ...
          case "gpt":
            if (
              element.op_type === "gpt_inquiry" ||
              element.op_type === "gpt_response"
            ) {
              ...
              console.log(
                `Message ${messageIndex}: time=${element.time}, content=${element.selected_text?.substring(0, 50)}...`,
              );
            }
            break;
        }
  ...
  const playMessages = (messagesToPlay: Message[]): void => {
    ...
    console.log(
      "Starting message playback with",
      messagesToPlay.length,
      "messages",
    );
    ...
    const syncMessages = () => {
      ...
      if (codePlayerRef.current) {
        ...
        const messagesToShow = allMessagesRef.current.filter(
          (msg) => msg.time <= currentTimeSec,
        );
  ...
  const handleSeek = async (percentage: number) => {
    ...
    console.log(
      `Seeking to element index ${seekIndex} (before time ${targetTimeSec.toFixed(2)}s)`,
    );
    ...
    editor.operation(() => {
      if (editorStateIndex >= 0 && data[editorStateIndex].current_editor) {
        const lines = parseEditorStateArray(
          data[editorStateIndex].current_editor,
        );
        const stateText = lines.join("\n");
        editor.setValue(stateText);
        console.log(
          `Set editor to state from index ${editorStateIndex} (time ${data[editorStateIndex]?.time || 0}s)`,
        );
      } else {
    ...
    console.log(
      `Built new recording from index ${recordingStartIndex} (time ${data[recordingStartIndex]?.time || 0}s)`,
    );
    ...
    const messagesToShow = allMessagesRef.current.filter(
      (msg) => msg.time <= targetTimeSec,
    );
    setMessReplay(messagesToShow);
    console.log(
      `Showing ${messagesToShow.length} messages up to seek position`,
    );
  ...
  return (
    <>
      ...
      <main
        className={`px-6 py-5 flex transition-all duration-300 ${
          isWordCountGraphExpanded
            ? "h-[calc(100vh-19rem)]"
            : "h-[calc(100vh-9rem)]"
        }`}
      >
          ...
          >
            ...
            >
              ...
              <Toast
                ...
                onClose={() => {
                  setShowCopyToast(false);
                  copyToastActiveRef.current = false;
                }}
                duration={2000}
                position="editor"
              />
      ...
      <div
        className={`fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg px-6 flex flex-col justify-end transition-all duration-300 ${
        ...
        >
          <div className="p-4">
            <Legend
              ...
              onCopyEventsToggle={setShowCopyEvents}
            />
          </div>
        </div>
