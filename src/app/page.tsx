...
async function loadCSV() {
  try {
    // Fetch the CSV file
    const response = await fetch("/data/replay_test.csv");
...
const handleLoadArrays = async (): Promise<void> => {
  const data = (await loadCSV()) as CSVdata;
  if (data) {
    ...
    if (data[0] && data[0].current_editor) {
      try {
        ...
        const initialState = initialLines.join('\n');
...
useEffect(() => {
  ...
  const checkEditor = setInterval(() => {
    ...
    if (editor) {
      console.log("Editor ready, loading participant 1");
